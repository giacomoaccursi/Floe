...
/**
 * Property-based tests for Primary Key validation
 * Feature: spark-etl-framework, Property 9: Primary Key Uniqueness Enforcement
...
object PKValidationProperties extends Properties("PKValidation") {
  ...
  val recordsWithDuplicatesGen: Gen[(Seq[TestRecord], Set[Int])] = for {
    ...
    val duplicatedIds = indicesToDuplicate.map(idx => uniqueRecords(idx).id).toSet
   ...
   * For any dataset with duplicate Primary Keys, all duplicate records should be rejected
   */
    ...
    case ((records, duplicatedIds), flowConfig) =>
      Try {
        if (records.isEmpty || duplicatedIds.isEmpty) {
          ...
          val duplicateRecordCount = records.count(r => duplicatedIds.contains(r.id))
  ...
  /**
   * Property 9: Primary Key Uniqueness Enforcement - Unique Records Pass
   * For any dataset with all unique Primary Keys, no records should be rejected for PK violation
   */
  ...
  /**
   * Property 9: Primary Key Uniqueness Enforcement - Composite PK
   * For any dataset with composite Primary Keys, duplicates should be identified correctly
   */
    ...
    Try {
      ...
      val flowConfig = FlowConfig(
        ...
        schema = SchemaConfig(
          ...
          columns = Seq(
            ColumnConfig("id", "int", nullable = false, None, "ID column"),
            ColumnConfig("name", "string", nullable = false, None, "Name column"),
            ColumnConfig("value", "double", nullable = true, None, "Value column")
          )
        ),
        loadMode = LoadModeConfig(
      ...
      val validHasUniqueKeys = {
        val validCount = result.valid.count()
        val distinctCount = result.valid.select("id", "name").distinct().count()
        validCount == distinctCount
      }
  ...
  /**
   * Property 9: Primary Key Uniqueness Enforcement - Empty DataFrame
   * For any empty DataFrame, PK validation should pass without errors
   */
  ...
  /**
   * Property 9: Primary Key Uniqueness Enforcement - Rejection Metadata Completeness
   * For any records rejected due to PK duplication, all metadata fields should be present
   */
          ...
          result.rejected.exists { rejectedDf =>
            ...
            val reasonMentionsPK = rejectedDf
              ...
              .forall { row =>
                ...
                reason.toLowerCase.contains("primary key") || reason.toLowerCase.contains("duplicate")
              }
  ...
  /**
   * Property 9: Primary Key Uniqueness Enforcement - Multiple Duplicates of Same Key
   * For any dataset where a PK appears multiple times, all occurrences should be rejected
   */
