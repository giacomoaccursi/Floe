...
class CustomValidatorTest extends AnyFlatSpec with Matchers {
  ...
  it should "pass configuration to the validator" in {
    val df = sampleRecords.toDF()
  ...
  it should "call configure() on ConfigurableValidator" in {
    val df = sampleRecords.toDF()
    ...
    val validator = ValidatorFactory.create(rule)
    validator shouldBe a[ConfigurableValidator]
  ...
  it should "throw ValidationConfigException for missing class name" in {
    val rule = ValidationRule(
      ...
      description = Some("Test missing class"),
      skipNull = Some(false),
      onFailure = OnFailureAction.Reject
    )
  ...
  it should "load multiple custom validators independently" in {
    val df = sampleRecords.toDF()

    val rule1 = customRule()
    ...
    validator1 should not be validator2

    val result1 = validator1.validate(df, rule1)
    val result2 = validator2.validate(df, rule2)

    result1.valid should not be null
    result2.valid should not be null
  }
