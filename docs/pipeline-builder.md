...
| `icebergMetadata` | `Option[IcebergFlowMetadata]` | Iceberg snapshot metadata (see [iceberg-integration.md](iceberg-integration.md)) |
...
Registers an additional table created during transformation. The table is saved to the output path and made available for DAG aggregation.
...
Tables registered with `dagMetadata` are automatically discovered by the DAG aggregation module when auto-discovery is enabled. See [dag-aggregation.md](dag-aggregation.md) for details.
