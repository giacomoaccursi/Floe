...
/** Property-based tests for dependency graph construction and topological sort
  * Feature: spark-etl-framework, Property 4: Dependency Graph Validity Feature:
  * spark-etl-framework, Property 5: Topological Sort Correctness Validates:
...
object DependencyGraphProperties extends Properties("DependencyGraph") {
  ...
  def flowWithDependenciesGen(
      flowName: String,
      referencedFlows: Seq[String]
  ): Gen[FlowConfig] = for {
    description <- Gen.alphaNumStr
    version <- Gen.oneOf("1.0", "1.1", "2.0")
    owner <- Gen.alphaNumStr.suchThat(_.nonEmpty)
  ...
  val validFlowDAGGen: Gen[Seq[FlowConfig]] = for {
    ...
    restFlows <- flowNames.tail.foldLeft(Gen.const(Seq.empty[FlowConfig])) {
      (genAcc, flowName) =>
        for {
          acc <- genAcc
          ...
          flow <-
            if (hasDeps && availableRefs.nonEmpty) {
  ...
  /** Property 4: Dependency Graph Validity For any set of flows with Foreign
    * Key relationships, the Flow_Orchestrator should build a dependency graph
    * where every FK dependency is represented as an edge.
    */
  property("dependency_graph_contains_all_fk_edges") =
    forAll(validFlowDAGGen, globalConfigGen) { (flows, globalConfig) =>
      ...
      val allFKsRepresented = flows.forall { flow =>
        val expectedDependencies =
          flow.validation.foreignKeys.map(_.references.flow).toSet
      ...
      val allFlowsInGraph =
        flows.forall(flow => dependencyGraph.contains(flow.name))

      allFKsRepresented && allFlowsInGraph
    }

  /** Property 5: Topological Sort Correctness For any valid dependency graph,
    * the topological sort should produce an execution order where all
    * dependencies of a flow are executed before that flow.
    */
  property("topological_sort_respects_dependencies") =
    forAll(validFlowDAGGen, globalConfigGen) { (flows, globalConfig) =>
      ...
      try {
        ...
        val dependenciesRespected = flows.forall { flow =>
          ...
          flow.validation.foreignKeys.forall { fk =>
            val depPosition =
              executionPosition.getOrElse(fk.references.flow, Int.MaxValue)
            depPosition < flowPosition
          }
        }

        // Verify all flows are in the execution order
        val allFlowsIncluded =
          flows.forall(flow => executionPosition.contains(flow.name))
  ...
  /** Property 5: Topological Sort Detects Circular Dependencies For any flows
    * with circular dependencies, topological sort should throw
  ...
  property("topological_sort_detects_circular_dependencies") = {
    ...
    val orchestrator = FlowOrchestrator(globalConfig, Seq(flowA, flowB, flowC))
  ...
  /** Property 5: Empty Flow List Produces Empty Execution Plan
    */
  property("empty_flows_produces_empty_plan") = forAll(globalConfigGen) {
    globalConfig =>
  ...
  /** Property 5: Single Flow With No Dependencies Produces Single Group
    */
  property("single_flow_produces_single_group") =
    forAll(flowNameGen, globalConfigGen) { (flowName, globalConfig) =>
