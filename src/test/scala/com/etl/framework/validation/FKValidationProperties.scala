...
/** Property-based tests for Foreign Key validation Feature:
  * spark-etl-framework, Property 10: Foreign Key Integrity Enforcement
  * Validates: Requirements 7.1
  *
  * Test configuration: Uses expensiveTestCount (10 cases) due to Spark
  * operations
  */
object FKValidationProperties extends Properties("FKValidation") {
  ...
  def childRecordsWithOrphansGen(
    ...
    validRecords <- Gen.listOfN(validSize, Gen.choose(1, validSize)).map {
      childIds =>
        childIds.zipWithIndex.map { case (_, idx) =>
          val parentId = parentIds(scala.util.Random.nextInt(parentIds.size))
          ChildRecord(idx + 1, parentId, s"child_$idx")
    ...
    orphanRecords <- Gen.listOfN(orphanSize, Gen.choose(1, orphanSize)).map {
      childIds =>
        childIds.zipWithIndex.map { case (_, idx) =>
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Orphan Records Rejected
    * For any dataset with Foreign Keys that don't exist in the referenced
    * table, all orphan records should be rejected
    */
  property("orphan_fk_records_all_rejected") = forAll(uniqueParentRecordsGen) {
    parentRecords =>
      forAll(childRecordsWithOrphansGen(parentRecords.map(_.parent_id))) {
        case (childRecords, orphanIds) =>
          forAll(
            flowConfigWithFKGen(
              "fk_parent",
              "parent_id",
            ...
            Try {
              if (
                parentRecords.isEmpty || childRecords.isEmpty || orphanIds.isEmpty
              ) {
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Valid FKs Pass For any
    * dataset with all valid Foreign Keys, no records should be rejected for FK
    * violation
    */
  property("valid_fk_records_all_pass") = forAll(uniqueParentRecordsGen) {
    parentRecords =>
      forAll(childRecordsWithValidFKsGen(parentRecords.map(_.parent_id))) {
        childRecords =>
          forAll(
            flowConfigWithFKGen(
              "fk_parent",
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Multiple FKs For any
    * dataset with multiple Foreign Keys, all FK constraints should be enforced
    */
  property("multiple_fk_constraints_enforced") =
    forAll(uniqueParentRecordsGen, uniqueParentRecordsGen) {
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Empty Parent Table For
    * any dataset when the parent table is empty, all child records should be
    * rejected
    */
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Null FK Values For any
    * dataset with null FK values, those records should be rejected
    */
  ...
  /** Property 10: Foreign Key Integrity Enforcement - FK Rejection Metadata For
    * any records rejected due to FK violation, all metadata fields should be
    * present
    */
  property("fk_rejection_metadata_complete") = forAll(uniqueParentRecordsGen) {
    parentRecords =>
      forAll(childRecordsWithOrphansGen(parentRecords.map(_.parent_id))) {
        case (childRecords, orphanIds) =>
          forAll(
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Record Count Invariant
    ...
    */
  property("fk_validation_preserves_record_count") =
    forAll(uniqueParentRecordsGen) { parentRecords =>
      forAll(childRecordsWithOrphansGen(parentRecords.map(_.parent_id))) {
        case (childRecords, _) =>
          forAll(
            flowConfigWithFKGen(
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Missing Referenced Flow
    * Error For any FK configuration referencing a non-existent flow, validation
    * should throw exception
    */
  property("missing_referenced_flow_throws_exception") =
    forAll(Gen.choose(5, 20)) { childCount =>
      forAll(
        flowConfigWithFKGen(
          "fk_parent",
          "parent_id",
  ...
  /** Property 10: Foreign Key Integrity Enforcement - Composite FK For any
    * dataset with composite Foreign Keys, all FK columns should be validated
    * together
    */
        ...
        validation = ValidationConfig(
          ...
          foreignKeys = Seq(
            ForeignKeyConfig(
              ...
              column = "parent_id1",
              references =
                ReferenceConfig(flow = "parent_flow", column = "parent_id1")
            ),
            ForeignKeyConfig(
              name = "fk_composite_parent2",
              column = "parent_id2",
              references =
                ReferenceConfig(flow = "parent_flow", column = "parent_id2")
            )
          ),
          rules = Seq.empty
        ),
