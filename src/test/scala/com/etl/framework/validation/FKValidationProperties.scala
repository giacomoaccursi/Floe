 ...
 * Feature: spark-etl-framework, Property 10: Foreign Key Integrity Enforcement
...
object FKValidationProperties extends Properties("FKValidation") {
  ...
  val uniqueParentRecordsGen: Gen[Seq[ParentRecord]] = for {
    ...
    records.zipWithIndex.map { case (rec, idx) => rec.copy(parent_id = idx + 1) }
  }
  
  // Generator for child records with valid FKs
  def childRecordsWithValidFKsGen(parentIds: Seq[Int]): Gen[Seq[ChildRecord]] = for {
  ...
  def childRecordsWithOrphansGen(parentIds: Seq[Int]): Gen[(Seq[ChildRecord], Set[Int])] = for {
  ...
  def flowConfigWithFKGen(fkName: String, fkColumn: String, refFlow: String, refColumn: String): Gen[FlowConfig] = for {
    name <- Gen.alphaNumStr.suchThat(_.nonEmpty)
  } yield FlowConfig(
    name = name,
    ...
    schema = SchemaConfig(
      ...
      columns = Seq(
        ...
        ColumnConfig("parent_id", "int", nullable = false, None, "Parent ID (FK)"),
        ColumnConfig("child_name", "string", nullable = true, None, "Child name")
      )
    ),
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Orphan Records Rejected
   * For any dataset with Foreign Keys that don't exist in the referenced table,
  ...
  property("orphan_fk_records_all_rejected") = forAll(uniqueParentRecordsGen) { parentRecords =>
    forAll(childRecordsWithOrphansGen(parentRecords.map(_.parent_id))) { case (childRecords, orphanIds) =>
      forAll(flowConfigWithFKGen("fk_parent", "parent_id", "parent_flow", "parent_id")) { flowConfig =>
        Try {
          if (parentRecords.isEmpty || childRecords.isEmpty || orphanIds.isEmpty) {
            ...
            val orphanCount = childRecords.count(r => orphanIds.contains(r.child_id))
            ...
            val validHasNoOrphans = {
              val validChildIds = result.valid.select("child_id").collect().map(_.getInt(0)).toSet
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Valid FKs Pass
   * For any dataset with all valid Foreign Keys, no records should be rejected for FK violation
   */
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Multiple FKs
   * For any dataset with multiple Foreign Keys, all FK constraints should be enforced
   */
      ...
      Try {
        if (parent1Records.isEmpty || parent2Records.isEmpty) {
          ...
          val validChildren = (1 to 10).map { i =>
            (i, parent1Ids(scala.util.Random.nextInt(parent1Ids.size)), 
             parent2Ids(scala.util.Random.nextInt(parent2Ids.size)), s"child_$i")
          }
          ...
          val allChildren = validChildren ++ orphan1Children ++ orphan2Children
          val childDf = allChildren.toDF("child_id", "parent1_id", "parent2_id", "name")
          
          val parent1Df = parent1Records.toDF().withColumnRenamed("parent_id", "parent1_id")
          val parent2Df = parent2Records.toDF().withColumnRenamed("parent_id", "parent2_id")
          ...
          val flowConfig = FlowConfig(
            ...
            schema = SchemaConfig(
              ...
              columns = Seq(
                ColumnConfig("child_id", "int", nullable = false, None, "Child ID"),
                ColumnConfig("parent1_id", "int", nullable = false, None, "Parent 1 ID"),
                ColumnConfig("parent2_id", "int", nullable = false, None, "Parent 2 ID"),
            ...
            validation = ValidationConfig(
              ...
              foreignKeys = Seq(
                ForeignKeyConfig(
                  name = "fk_parent1",
                  column = "parent1_id",
                  references = ReferenceConfig(flow = "parent1_flow", column = "parent1_id")
                ),
                ForeignKeyConfig(
                  name = "fk_parent2",
                  column = "parent2_id",
                  references = ReferenceConfig(flow = "parent2_flow", column = "parent2_id")
                )
              ),
              rules = Seq.empty
            ),
          ...
          // All orphan records should be rejected
          val expectedRejectedCount = orphan1Children.size + orphan2Children.size
          val allOrphansRejected = result.rejected.exists { rejectedDf =>
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Empty Parent Table
   * For any dataset when the parent table is empty, all child records should be rejected
   */
  property("empty_parent_table_rejects_all_children") = forAll(Gen.choose(5, 20)) { childCount =>
    forAll(flowConfigWithFKGen("fk_parent", "parent_id", "parent_flow", "parent_id")) { flowConfig =>
      Try {
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Null FK Values
   * For any dataset with null FK values, those records should be rejected
   */
  property("null_fk_values_rejected") = forAll(uniqueParentRecordsGen, Gen.choose(5, 20)) { 
        ...
        Try {
          if (parentRecords.isEmpty) {
            ...
            val validChildren = (1 to childCount / 2).map { i =>
              ChildRecord(i, parentIds(scala.util.Random.nextInt(parentIds.size)), s"child_$i")
            }
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - FK Rejection Metadata
   * For any records rejected due to FK violation, all metadata fields should be present
   */
            ...
            result.rejected.exists { rejectedDf =>
              ...
              val reasonMentionsFK = rejectedDf
                ...
                .forall { row =>
                  ...
                  reason.toLowerCase.contains("foreign key") || reason.toLowerCase.contains("fk")
                }
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Record Count Invariant
   * For any dataset, input_count = valid_count + rejected_count
   */
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Missing Referenced Flow Error
   * For any FK configuration referencing a non-existent flow, validation should throw exception
   */
  property("missing_referenced_flow_throws_exception") = forAll(Gen.choose(5, 20)) { childCount =>
    forAll(flowConfigWithFKGen("fk_parent", "parent_id", "nonexistent_flow", "parent_id")) { flowConfig =>
      Try {
        val childRecords = (1 to childCount).map { i =>
          ChildRecord(i, i, s"child_$i")
        }
  ...
  /**
   * Property 10: Foreign Key Integrity Enforcement - Composite FK
   * For any dataset with composite Foreign Keys, all FK columns should be validated together
   */
  property("composite_fk_validation") = forAll(Gen.choose(5, 20)) { size =>
    Try {
      ...
      val childDf = allChildren.toDF("child_id", "parent_id1", "parent_id2", "child_name")
      ...
      val flowConfig = FlowConfig(
        ...
        schema = SchemaConfig(
          ...
          columns = Seq(
            ColumnConfig("child_id", "int", nullable = false, None, "Child ID"),
            ColumnConfig("parent_id1", "int", nullable = false, None, "Parent ID 1"),
            ColumnConfig("parent_id2", "int", nullable = false, None, "Parent ID 2"),
            ColumnConfig("child_name", "string", nullable = true, None, "Child name")
          )
        ),
        loadMode = LoadModeConfig(
