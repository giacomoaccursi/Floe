 ...
 * 
 * The Final Model represents the external data model used for export,
 * APIs, and downstream consumption.
 * 
 * @tparam F FinalModel type (must have an Encoder)
 * @param outputConfig Configuration for output format, path, partitioning, etc.
...
class FinalModelWriter[F: Encoder](
   ...
   * 
   * The output format, partitioning, and compression are determined
   * by the outputConfig.
   * 
  ...
  def write(dataset: Dataset[F], outputPath: String): Unit = {
    ...
    logger.info(s"Output format: ${outputConfig.format}")
    ...
    try {
      ...
      if (outputConfig.partitionBy.nonEmpty) {
        logger.info(s"Partitioning by: ${outputConfig.partitionBy.mkString(", ")}")
   ...
   * 
   * This is useful for writing the same final model to different
   * locations or formats (e.g., Parquet for storage, JSON for API).
   * 
   * @param dataset Dataset containing final model records
  ...
  def writeMultiple(dataset: Dataset[F], outputPaths: Seq[String]): Unit = {
    ...
    logger.info(s"Successfully wrote Final Model to all ${outputPaths.size} paths")
  }
}
...
object FinalModelWriter {
   ...
   * 
   * @param outputConfig Configuration for output format, partitioning, etc.
   * @param spark Implicit SparkSession
   * @tparam F FinalModel type
   * @return FinalModelWriter instance
   */
  def apply[F: Encoder](outputConfig: OutputConfig)(implicit spark: SparkSession): FinalModelWriter[F] = {
    logger.debug(s"Creating FinalModelWriter with format: ${outputConfig.format}")
   ...
   * @param spark Implicit SparkSession
   * @tparam F FinalModel type
   * @return FinalModelWriter instance with Parquet format
   */
   ...
   * @param spark Implicit SparkSession
   * @tparam F FinalModel type
   * @return FinalModelWriter instance with JSON format
   */
   ...
   * @param spark Implicit SparkSession
   * @tparam F FinalModel type
   * @return FinalModelWriter instance with CSV format
   */
