...
/** Property-based tests for additional table persistence Feature:
  * spark-etl-framework, Property 20: Additional Table Persistence Validates:
  * Requirements 13.1
  *
  * Test configuration: Uses expensiveTestCount (10 cases) due to file I/O and
  * Spark operations
  */
object AdditionalTablePersistenceProperties
    extends Properties("AdditionalTablePersistence") {
  ...
  /** Property 20: Additional Table Persistence For any transformation that
    * calls ctx.addTable(), the table should be automatically saved to Parquet
    * at the specified or default path.
    */
  property("additional_table_is_persisted_to_parquet") = forAll(
    ...
    // Execute flow
    val executor =
      new FlowExecutor(flowConfigWithTransform, globalConfig, Map.empty)
    ...
    val metadataPath =
      s"${globalConfig.paths.metadataPath}/$batchId/additional_tables/${tableName}.json"
  ...
  /** Property: Additional table with custom output path is persisted to
    * specified location For any transformation that calls ctx.addTable() with a
    * custom outputPath, the table should be saved to the specified path.
    */
  ...
  /** Property: Additional table metadata includes schema and statistics For any
    * additional table created, the metadata should contain schema information
  ...
  property("additional_table_metadata_includes_schema_and_statistics") = forAll(
    ...
    val metadataValid = Try {
      val metadataContent =
        new String(Files.readAllBytes(Paths.get(metadataPath)))
  ...
  /** Property: Additional table with dagMetadata includes join configuration
    * For any additional table created with dagMetadata, the metadata should
    * include primary key and join key information for Transformation
    * integration.
    */
  ...
  /** Property: Multiple additional tables can be created in single
    * transformation For any transformation that calls ctx.addTable() multiple
    * times, all tables should be persisted correctly.
    */
