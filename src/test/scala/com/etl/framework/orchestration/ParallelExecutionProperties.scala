...
/** Property-based tests for parallel execution of independent flows Feature:
  * spark-etl-framework, Property 6: Parallel Execution of Independent Flows
  * Validates: Requirements 3.3
  *
  * Test configuration: Uses minimalTestCount (5 cases) - parallel execution
  * tests are expensive
  */
object ParallelExecutionProperties extends Properties("ParallelExecution") {
  ...
  /** Property 6: Parallel Execution of Independent Flows For any set of
    * independent flows (no FK dependencies) with parallel execution enabled,
    * the flows should be grouped together for potential parallel execution.
    */
  property("independent_flows_grouped_for_parallel_execution") =
    forAll(Gen.choose(2, 5)) { flowCount =>
  ...
  /** Property 6: Sequential Execution When Parallel Disabled For any set of
    * independent flows with parallel execution disabled, the flows should be
    * marked for sequential execution.
    */
  property("independent_flows_sequential_when_parallel_disabled") =
    forAll(Gen.choose(2, 5)) { flowCount =>
  ...
  /** Property 6: Dependent Flows Not Grouped Together For any flows with
    * dependencies, they should not be in the same execution group.
    */
  property("dependent_flows_in_separate_groups") = forAll(Gen.const(())) { _ =>
    ...
    try {
      ...
      val orchestrator = FlowOrchestrator(globalConfig, Seq(flowA, flowB))
      val plan = orchestrator.buildExecutionPlan()
  ...
  /** Property 6: All Flows Executed Regardless of Grouping For any set of
    * flows, all flows should be included in the execution plan.
    */
  property("all_flows_included_in_execution_plan") = forAll(Gen.choose(1, 5)) {
    flowCount =>
