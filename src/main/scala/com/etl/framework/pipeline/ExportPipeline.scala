...
class ExportPipeline[B: Encoder, F: Encoder] private(
  ...
  /**
   * Executes Export pipeline
   * Returns Dataset[F] (Final Model)
   */
 ...
 */
class ExportPipelineBuilder[B: Encoder, F: Encoder](implicit spark: SparkSession) {
   ...
   * 
   * @param path Path to Batch Model (Parquet format)
  ...
  /**
   * Sets the Batch Model dataset directly
   * Alternative to withBatchModelPath for in-memory datasets
   * 
   * @param dataset Batch Model dataset
   * @return This builder for chaining
   */
  def withBatchModelDataset(dataset: Dataset[B]): ExportPipelineBuilder[B, F] = {
  ...
  /**
   * Specifies the Final Model type (for type safety)
   * This is a no-op method that helps with type inference
   * 
   * @return This builder for chaining
   */
   ...
   * 
   * @param mapper FinalModelMapper instance
   * @return This builder for chaining
   */
  def withMapper(mapper: FinalModelMapper[B, F]): ExportPipelineBuilder[B, F] = {
  ...
  /**
   * Loads the mapper by class name using reflection
   * The mapper class must implement FinalModelMapper[B, F]
   * 
  ...
  /**
   * Sets the output path for writing the Final Model
   * If not set, the Final Model will not be written to disk
   * 
  ...
  /**
   * Sets the output format (convenience method)
   * Creates a default OutputConfig with the specified format
   * 
   * @param format Output format (parquet, json, csv, etc.)
   * @return This builder for chaining
   */
  def withOutputFormat(format: String): ExportPipelineBuilder[B, F] = {
    ...
    val options = if (format.toLowerCase == "csv") {
      Map("header" -> "true")
    } else {
      Map.empty[String, String]
    }
    
    this.outputConfigOpt = Some(OutputConfig(
      path = None,
      rejectedPath = None,
      format = format,
  ...
  def withPartitioning(columns: String*): ExportPipelineBuilder[B, F] = {
    ...
    val currentConfig = outputConfigOpt.getOrElse(OutputConfig(
      path = None,
      rejectedPath = None,
      format = "parquet",
      partitionBy = Seq.empty,
   ...
   * 
   * @return Configured ExportPipeline
  ...
  def build(): ExportPipeline[B, F] = {
    ...
    val outputConfig = outputConfigOpt.getOrElse {
      logger.info("No output config provided, using default Parquet configuration")
      OutputConfig(
        path = None,
        rejectedPath = None,
        format = "parquet",
...
object ExportPipeline {
  ...
  private[pipeline] def create[B: Encoder, F: Encoder](
    ...
    new ExportPipeline[B, F](batchModelPath, batchModelDataset, mapper, outputConfig, outputPath)
  }
   ...
   * @return New ExportPipelineBuilder
   */
  def builder[B: Encoder, F: Encoder]()(implicit spark: SparkSession): ExportPipelineBuilder[B, F] = {
