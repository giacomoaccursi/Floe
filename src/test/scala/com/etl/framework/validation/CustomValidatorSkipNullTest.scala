...
/**
 * A custom validator that rejects all rows — used to verify skipNull behavior.
 * If skipNull works, NULL rows should NOT be passed to this validator and should survive.
 */
class RejectAllValidator extends Validator {
  override def validate(df: DataFrame, rule: ValidationRule): ValidationStepResult = {
    val rejected = df.withColumn(ValidationColumns.REJECTION_CODE, lit("CUSTOM_REJECT"))
