...
/**
 * Property-based tests for nest join strategy
 * Feature: spark-etl-framework, Property 22: Nest Join Strategy
...
object NestJoinProperties extends Properties("NestJoin") {
  ...
  /**
   * Property 22: Nest Join Creates Nested Array Field
   * For any parent-child join with strategy "nest", the result should have a nested array field
  ...
  property("nest_join_creates_nested_array_field") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(s"Exception in nest_join_creates_nested_array_field: ${e.getMessage}")
          e.printStackTrace()
          false
      }
    }
  ...
  /**
   * Property 22: Nest Join Preserves Parent Records (Left Outer)
   * For any parent-child left outer join with nest strategy, all parent records should be preserved.
   */
  property("nest_join_preserves_parent_records_left_outer") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(s"Exception in nest_join_preserves_parent_records_left_outer: ${e.getMessage}")
          e.printStackTrace()
          false
      }
    }
  ...
  /**
   * Property 22: Nest Join Groups Child Records by Parent
   * For any parent-child join with nest strategy, each parent should have an array containing
   * only its related child records.
   */
  property("nest_join_groups_child_records_by_parent") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        val expectedChildrenPerParent = childRecords.groupBy(_.parent_id).mapValues(_.size)
        ...
        case e: Exception =>
          println(s"Exception in nest_join_groups_child_records_by_parent: ${e.getMessage}")
          e.printStackTrace()
          false
      }
    }
  ...
  /**
   * Property 22: Nest Join Preserves Parent Columns
   * For any parent-child join with nest strategy, all parent columns should be preserved in the result.
   */
  property("nest_join_preserves_parent_columns") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(s"Exception in nest_join_preserves_parent_columns: ${e.getMessage}")
          e.printStackTrace()
          false
      }
    }
  ...
  /**
   * Property 22: Nest Join With Empty Child Produces Empty Arrays
   * For any parent records with no matching children, the nested array should be empty (not null).
   */
  property("nest_join_with_no_children_produces_empty_arrays") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(s"Exception in nest_join_with_no_children_produces_empty_arrays: ${e.getMessage}")
          e.printStackTrace()
          false
      }
    }
