...
A primary key definition is required. If `primaryKey` is empty, the validator throws a `ValidationConfigException`. This also means that flows without a natural primary key still need one defined (even if it's a synthetic key added in a pre-validation transformation).
...
- If the referenced flow has not been processed yet, a `ValidationConfigException` is thrown. Flow execution order matters.
- Rejection code: `FK_VIOLATION`.

The `onOrphan` field does not affect intra-batch validation — it controls post-batch orphan detection behavior. See [orphan-detection.md](orphan-detection.md).
...
Validates that column values belong to a predefined set of allowed values. Domains are defined in a separate `domains.yaml` file:
...
For validation logic that cannot be expressed with regex, range, or domain rules, you can provide a custom validator class:
