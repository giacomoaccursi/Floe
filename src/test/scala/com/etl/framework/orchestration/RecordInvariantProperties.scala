...
/** Property-based tests for record invariant (CRITICAL) Feature: spark-etl-framework, Property 29: Record Invariant
  * Feature: spark-etl-framework, Property 30: Invariant Violation Exception Validates: Requirements 22.1, 22.2
  *
...
object RecordInvariantProperties extends Properties("RecordInvariant") {
  ...
  /** Property 29: Record Invariant (CRITICAL) For any flow execution, the invariant input_count = valid_count +
    * rejected_count must hold.
    */
  ...
  /** Property 30: Invariant Violation Exception For any flow execution where the invariant is violated, an
    * InvariantViolationException should be thrown.
    */
  ...
  /** Property: Invariant holds after filtering For any dataset that is filtered, the invariant should hold where input
  ...
  /** Property: Invariant holds after validation split For any dataset that is split into valid and rejected, the
    * invariant input = valid + rejected should hold.
    */
  ...
  /** Property: Invariant holds with empty rejected set For any dataset where all records are valid (no rejections), the
    * invariant input = valid + 0 should hold.
    */
  ...
  /** Property: Invariant holds with all rejected For any dataset where all records are rejected (no valid records), the
    * invariant input = 0 + rejected should hold.
    */
  ...
  /** Property: Invariant violation message is descriptive For any invariant violation, the exception message should
    * contain the actual counts for debugging.
    */
  ...
  /** Property: Invariant is checked independently per flow For any multiple flows, each flow's invariant should be
  ...
  /** Property: Invariant holds after transformations For any dataset that undergoes transformations (filter, map,
    * etc.), if we track input and output counts, the invariant should hold.
    */
