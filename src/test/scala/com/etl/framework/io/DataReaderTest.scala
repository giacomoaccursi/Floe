...
class DataReaderTest extends AnyFlatSpec with Matchers {
  ...
  "DataReader" should "read CSV files with comma delimiter" in {
    ...
    try {
      ...
      val config = SourceConfig(SourceType.File, tempPath, FileFormat.CSV,
        Map("delimiter" -> ",", "header" -> "true", "inferSchema" -> "true"), None)
  ...
  it should "read Parquet files" in {
    ...
    try {
      ...
      val config = SourceConfig(SourceType.File, tempPath, FileFormat.Parquet, Map.empty, None)
      val reader = DataReaderFactory.create(config)
      val df = reader.read()
  ...
  it should "read JSON files" in {
    ...
    try {
      ...
      val config = SourceConfig(SourceType.File, tempPath, FileFormat.JSON, Map.empty, None)
      val reader = DataReaderFactory.create(config)
      val df = reader.read()
  ...
  it should "read files matching a file pattern" in {
    ...
    try {
      ...
      val config = SourceConfig(SourceType.File, tempPath, FileFormat.CSV,
        Map("header" -> "true", "inferSchema" -> "true"), Some("*.csv"))
      val reader = DataReaderFactory.create(config)
