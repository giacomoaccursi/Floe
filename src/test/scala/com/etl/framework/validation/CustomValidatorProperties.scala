...
/**
 * Property-based tests for custom validator dynamic loading
 * Feature: spark-etl-framework, Property 32: Custom Validator Dynamic Loading
...
object CustomValidatorProperties extends Properties("CustomValidator") {
  ...
  /**
   * Property 32: Custom Validator Dynamic Loading - Valid Class Name
   * For any valid custom validator class name, the Framework should successfully load the validator
   */
  property("custom_validator_loads_successfully") = forAll(testRecordsGen) { records =>
    Try {
      if (records.isEmpty) {
  ...
  /**
   * Property 32: Custom Validator Dynamic Loading - Validator Execution
   * For any loaded custom validator, it should execute validation correctly
   */
  property("custom_validator_executes_validation") = forAll(testRecordsGen) { records =>
    Try {
      if (records.isEmpty) {
        true // Skip empty test cases
      } else {
  ...
  /**
   * Property 32: Custom Validator Dynamic Loading - Configuration Passed
   * For any custom validator with configuration, the configuration should be passed via configure()
   */
  property("custom_validator_receives_configuration") = forAll(Gen.choose(1, 100)) { threshold =>
    Try {
      val records = Seq(
  ...
  /**
   * Property 32: Custom Validator Dynamic Loading - Configurable Validator
   * For any custom validator implementing ConfigurableValidator, configure() should be called
   */
  property("configurable_validator_configure_called") = forAll(testRecordsGen) { records =>
    Try {
      if (records.isEmpty) {
        true // Skip empty test cases
      } else {
   ...
   * For any set of different custom validator class names, each should load independently
   */
  property("multiple_custom_validators_load_independently") = forAll(testRecordsGen) { records =>
    Try {
      if (records.isEmpty) {
        true // Skip empty test cases
      } else {
  ...
  /**
   * Property 32: Custom Validator Dynamic Loading - Rejection Metadata
   * For any custom validator that rejects records, rejection metadata should be present
   */
  property("custom_validator_rejection_metadata") = forAll(testRecordsGen) { records =>
    Try {
      if (records.isEmpty) {
        true // Skip empty test cases
      } else {
  ...
  /**
   * Property 32: Custom Validator Dynamic Loading - Record Count Invariant
   * For any custom validator execution, input_count = valid_count + rejected_count
   */
  property("custom_validator_preserves_record_count") = forAll(testRecordsGen) { records =>
    Try {
      if (records.isEmpty) {
        true // Skip empty test cases
      } else {
...
/**
 * Test custom validator for property testing
 * Rejects records where value.length > threshold
 */
...
/**
 * Test configurable custom validator for property testing
 * Rejects records where value doesn't start with prefix or end with suffix
 */
