...
/** Property-based tests for batch metadata generation Feature:
  * spark-etl-framework, Property 25: Batch ID Uniqueness Feature:
  * spark-etl-framework, Property 26: Metadata Recording Completeness Validates:
  * Requirements 19.1, 19.2
  *
  * Test configuration: Uses minimalTestCount (5 cases) - metadata tests are
  * expensive
  */
object BatchMetadataProperties extends Properties("BatchMetadata") {
  ...
  /** Property 25: Batch ID Uniqueness For any two batch executions, the
    * generated batch_ids should be unique.
    */
  property("batch_ids_are_unique_across_executions") =
    forAll(Gen.choose(2, 5)) { executionCount =>
  ...
  /** Property 25: Batch ID Format Consistency For any batch execution, the
    * batch_id should follow the configured format.
    */
  property("batch_id_follows_configured_format") =
    forAll(Gen.oneOf("timestamp", "datetime", "yyyyMMdd_HHmmss")) { format =>
  ...
  /** Property 26: Metadata Recording Completeness For any flow execution, the
    * metadata should contain input_records, valid_records, rejected_records,
    * and rejection_rate.
    */
  property("metadata_contains_required_fields") = forAll(Gen.choose(1, 3)) {
    flowCount =>
      val tempDir =
  ...
  /** Property 26: Per-Flow Metadata Contains Required Fields For any flow
    * execution, the per-flow metadata should contain all required fields.
    */
  ...
  /** Property 26: Latest Symlink Points to Most Recent Batch For any batch
    * execution, the "latest" symlink should point to the most recent batch.
    */
  property("latest_symlink_points_to_most_recent_batch") = {
    ...
    try {
      ...
      val pointsToSecondBatch =
        if (latestExists && Files.isSymbolicLink(latestPath)) {
  ...
  /** Property 26: Metadata Records Rejection Reasons For any flow with rejected
    * records, the metadata should contain rejection reasons with counts.
    */
  ...
  /** Property 26: Empty Flow List Produces Valid Metadata For any empty flow
    * list, metadata should still be generated with appropriate values.
    */
