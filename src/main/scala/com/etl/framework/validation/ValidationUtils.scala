...
object ValidationUtils {
  ...
  /** Combines two optional DataFrames using union Used to accumulate rejected records from multiple validation steps
    *
    * @param existing
    *   Previously accumulated rejected records
    * @param newRejected
    *   Newly rejected records from current validation step
    * @return
    *   Combined rejected records, or None if both are empty
    */
  def combineRejected(
  ...
  /** Adds rejection metadata columns to a DataFrame
    ...
    * @param rejectionCode
    *   Code identifying the type of rejection
    * @param rejectionReason
    *   Human-readable reason for rejection
    * @param validationStep
    *   Name of the validation step that rejected the records
    * @return
  ...
  /** Creates a ValidationStepResult with no rejections Convenience method for early returns when validation passes
    */
  def validResult(df: DataFrame): ValidationStepResult = {
    ValidationStepResult(df, None)
  }

  /** Creates a ValidationStepResult with rejections Convenience method for creating results with rejected records
    */
