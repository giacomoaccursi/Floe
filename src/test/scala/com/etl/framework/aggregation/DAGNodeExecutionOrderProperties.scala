...
/** Property-based tests for DAG node execution order Feature:
  * spark-etl-framework, Property 21: DAG Node Execution Order Validates:
  * Requirements 14.2, 14.3
  *
  * Test configuration: Uses standardTestCount (20 cases)
  */
object DAGNodeExecutionOrderProperties
    extends Properties("DAGNodeExecutionOrder") {
  ...
  def nodeWithDependenciesGen(
    ...
    joinConfig <-
      if (hasJoin && selectedDeps.nonEmpty) {
        Gen.const(
          Some(
            JoinConfig(
  ...
  val validNodeDAGGen: Gen[Seq[DAGNode]] = for {
    ...
    restNodes <- nodeIds.tail
      .zip(sourceFlows.tail)
      .foldLeft(Gen.const(Seq.empty[DAGNode])) {
  ...
  def aggregationConfigGen(nodes: Seq[DAGNode]): Gen[AggregationConfig] =
    Gen.const(
      AggregationConfig(
  ...
  /** Property 21: DAG Node Execution Order For any DAG with node dependencies,
    * the execution order should ensure that all dependencies of a node are
    * executed before that node.
    */
  property("dag_execution_order_respects_dependencies") =
    forAll(validNodeDAGGen, globalConfigGen) { (nodes, globalConfig) =>
      ...
      try {
        ...
        val allNodesIncluded =
          nodes.forall(node => executionPosition.contains(node.id))
  ...
  /** Property 21: DAG Dependency Graph Contains All Edges For any set of DAG
    * nodes with dependencies, the dependency graph should contain all
    * dependency relationships.
    */
  property("dag_dependency_graph_contains_all_edges") =
    forAll(validNodeDAGGen, globalConfigGen) { (nodes, globalConfig) =>
      val dagConfig = aggregationConfigGen(nodes).sample.get
      ...
      val allNodesInGraph =
        nodes.forall(node => dependencyGraph.contains(node.id))

      allDepsRepresented && allNodesInGraph
    }

  /** Property 21: Topological Sort Detects Circular Dependencies in DAG For any
    * DAG nodes with circular dependencies, topological sort should throw
  ...
  /** Property 21: Empty Node List Produces Empty Execution Plan
    */
  property("empty_nodes_produces_empty_plan") = forAll(globalConfigGen) {
    globalConfig =>
      val dagConfig = AggregationConfig(
        name = "empty_test",
  ...
  /** Property 21: Single Node With No Dependencies Produces Single Group
    */
  property("single_node_produces_single_group") =
    forAll(nodeIdGen, globalConfigGen) { (nodeId, globalConfig) =>
      val node = DAGNode(
        id = nodeId,
