...
/** Property-based tests for schema validation Feature: spark-etl-framework,
  * Property 8: Schema Validation Completeness Validates: Requirements 5.1, 5.2
  */
object SchemaValidationProperties extends Properties("SchemaValidation") {
  ...
  def createDataFrame(columns: Seq[String], rowCount: Int): DataFrame = {
    ...
    val schema = StructType(
      columns.map(col => StructField(col, StringType, nullable = true))
    )
    spark.createDataFrame(spark.sparkContext.parallelize(rows), schema)
  }

  // Helper function to create DataFrame matching schema config
  def createMatchingDataFrame(
  ...
  /** Property 8: Schema Validation Completeness - Missing Required Columns For
    * any data with missing required columns, the Validation_Engine should
    * reject all records Note: Currently the ValidationEngine throws an
    * exception when trying to validate not-null constraints on missing columns.
    * This test verifies that schema validation detects the issue.
    */
  property("missing_required_columns_rejects_all") =
    forAll(flowConfigGen, Gen.choose(1, 50)) { (flowConfig, rowCount) =>
      Try {
        ...
        if (requiredColumns.size < 2 || rowCount <= 0) {
          ...
          val missingColumnIndex =
            scala.util.Random.nextInt(requiredColumns.size)
          val incompleteColumns =
            requiredColumns.patch(missingColumnIndex, Nil, 1)
          ...
          val correctCode = result.rejected.exists { rejectedDf =>
            rejectedDf.select(REJECTION_CODE).distinct().collect().forall {
              row =>
  ...
  /** Property 8: Schema Validation Completeness - All Required Columns Present
    * For any data with all required columns present, the Validation_Engine
    * should pass schema validation
    */
  property("all_required_columns_present_passes") =
    forAll(flowConfigGen, Gen.choose(1, 50)) { (flowConfig, rowCount) =>
      Try {
        ...
        val noSchemaRejections = result.rejected.forall { rejectedDf =>
          !rejectedDf
            .filter(col(REJECTION_CODE) === "SCHEMA_VALIDATION_FAILED")
            .isEmpty == false
        }
  ...
  /** Property 8: Schema Validation Completeness - Multiple Missing Columns For
    * any data with multiple missing required columns, all records should be
    * rejected
    */
  property("multiple_missing_columns_rejects_all") =
    forAll(flowConfigGen, Gen.choose(1, 50)) { (flowConfig, rowCount) =>
      Try {
        val requiredColumns = flowConfig.schema.columns.map(_.name)
        ...
        if (requiredColumns.size < 3 || rowCount <= 0) {
          ...
          val reasonMentionsMissing = result.rejected.exists { rejectedDf =>
            rejectedDf.select(REJECTION_REASON).distinct().collect().forall {
              row =>
  ...
  /** Property 8: Schema Validation Completeness - Extra Columns Allowed For any
    * data with extra columns beyond required, validation should pass if
    * allowExtraColumns is true
    */
  property("extra_columns_allowed_when_configured") =
    forAll(flowConfigGen, Gen.choose(1, 50)) { (flowConfig, rowCount) =>
      Try {
  ...
  /** Property 8: Schema Validation Completeness - Empty DataFrame For any empty
    * DataFrame with missing required columns, validation should still reject
    */
  property("empty_dataframe_missing_columns_rejects") = forAll(flowConfigGen) {
    flowConfig =>
      Try {
        val requiredColumns = flowConfig.schema.columns.map(_.name)
  ...
  /** Property 8: Schema Validation Completeness - Rejection Metadata
    * Completeness For any data rejected due to schema validation, all rejection
    * metadata fields should be present
    */
  property("schema_rejection_metadata_complete") =
    forAll(flowConfigGen, Gen.choose(1, 50)) { (flowConfig, rowCount) =>
      Try {
        val requiredColumns = flowConfig.schema.columns.map(_.name)
  ...
  /** Property 8: Schema Validation Completeness - Schema Enforcement Disabled
    * For any data when enforceSchema is false, schema validation should be
    * skipped
    */
  property("schema_enforcement_disabled_skips_validation") =
    forAll(flowConfigGen, Gen.choose(1, 50)) { (flowConfig, rowCount) =>
      Try {
