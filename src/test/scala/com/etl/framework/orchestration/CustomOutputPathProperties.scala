...
/**
 * Property-based tests for custom output paths
 * Feature: spark-etl-framework, Property 28: Custom Output Path Respect
...
object CustomOutputPathProperties extends Properties("CustomOutputPath") {
  ...
  /**
   * Property 28: Custom Output Path Respect
   * For any flow with custom output.path configured, the validated data should be
   * saved to the specified path, not the default path.
   */
  ...
  /**
   * Property: Custom rejected path is respected
   * For any flow with custom output.rejectedPath configured, the rejected data should be
   * saved to the specified path, not the default rejected path.
   */
  ...
  /**
   * Property: Default path is used when no custom path specified
   * For any flow without custom output.path, the validated data should be
   * saved to the default path.
   */
  property("default_path_used_when_no_custom_path") = forAll(globalConfigGen) { globalConfig =>
  ...
  /**
   * Property: Custom path supports environment variables
   * For any flow with custom path containing environment variable placeholders,
   * the path should be valid and different from default.
   */
  ...
  /**
   * Property: Custom path can be on different storage
   * For any flow with custom path on different storage (HDFS, S3, local),
   * the path should be valid and use the correct protocol.
   */
  ...
  /**
   * Property: Partitioning is applied when configured
   * For any flow with partitionBy configured, the output should respect partitioning.
   */
