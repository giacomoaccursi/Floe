...
/** Property-based tests for Primary Key validation Feature:
  * spark-etl-framework, Property 9: Primary Key Uniqueness Enforcement
...
object PKValidationProperties extends Properties("PKValidation") {
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - All Duplicates Rejected
    * For any dataset with duplicate Primary Keys, all duplicate records should
    * be rejected
    */
  property("duplicate_pk_records_all_rejected") =
    forAll(recordsWithDuplicatesGen, flowConfigWithPKGen(Seq("id"))) {
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - Unique Records Pass For
    * any dataset with all unique Primary Keys, no records should be rejected
    * for PK violation
    */
  property("unique_pk_records_all_pass") =
    forAll(uniqueRecordsGen, flowConfigWithPKGen(Seq("id"))) {
      (records, flowConfig) =>
        Try {
          if (records.isEmpty) {
            true // Skip empty test cases
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - Composite PK For any
    * dataset with composite Primary Keys, duplicates should be identified
    * correctly
    */
  property("composite_pk_duplicate_detection") = forAll(Gen.choose(5, 30)) {
    size =>
      Try {
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - Empty DataFrame For any
    * empty DataFrame, PK validation should pass without errors
    */
  property("empty_dataframe_pk_validation") =
    forAll(flowConfigWithPKGen(Seq("id"))) { flowConfig =>
      Try {
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - Rejection Metadata
    * Completeness For any records rejected due to PK duplication, all metadata
    * fields should be present
    */
  property("pk_rejection_metadata_complete") =
    forAll(recordsWithDuplicatesGen, flowConfigWithPKGen(Seq("id"))) {
      case ((records, duplicatedIds), flowConfig) =>
        Try {
          if (records.isEmpty || duplicatedIds.isEmpty) {
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - Multiple Duplicates of
    * Same Key For any dataset where a PK appears multiple times, all
    * occurrences should be rejected
    */
  property("multiple_duplicates_same_key_all_rejected") =
    forAll(Gen.choose(3, 10)) { duplicateCount =>
      Try {
  ...
  /** Property 9: Primary Key Uniqueness Enforcement - Record Count Invariant
    * For any dataset, input_count = valid_count + rejected_count
    */
  property("pk_validation_preserves_record_count") =
    forAll(recordsWithDuplicatesGen, flowConfigWithPKGen(Seq("id"))) {
