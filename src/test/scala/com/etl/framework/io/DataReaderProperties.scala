...
/** Property-based tests for data source reading Feature: spark-etl-framework,
  * Property 7: Data Source Reading Validates: Requirements 4.1, 4.2
  *
...
object DataReaderProperties extends Properties("DataReader") {
  ...
  def writeTestDataToFile(
    ...
    format match {
      ...
      case _ => // Should not happen in this test
    }
  }
  ...
  /** Property 7: Data Source Reading for CSV files For any valid CSV source
    * configuration, the Framework should successfully read the data and return
    * a DataFrame with the expected schema
    */
  property("csv_file_reading") = forAll(testDataGen, csvSourceConfigGen) {
    (data, sourceConfig) =>
      Try {
  ...
  /** Property 7: Data Source Reading for Parquet files For any valid Parquet
    * source configuration, the Framework should successfully read the data and
    * return a DataFrame with the expected schema
    */
  property("parquet_file_reading") =
    forAll(testDataGen, parquetSourceConfigGen) { (data, sourceConfig) =>
      Try {
        // Create temp directory for test data
  ...
  /** Property 7: Data Source Reading for JSON files For any valid JSON source
    * configuration, the Framework should successfully read the data and return
    * a DataFrame with the expected schema
    */
  property("json_file_reading") = forAll(testDataGen, jsonSourceConfigGen) {
    (data, sourceConfig) =>
      Try {
        // Create temp directory for test data
  ...
  /** Property 7: Data Source Reading with file patterns For any valid source
    * configuration with file pattern, the Framework should successfully read
    * matching files and return a DataFrame with the expected schema
    */
  ...
  // Removed obsolete tests for unsupported source type and file format
