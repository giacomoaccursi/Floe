...
/** Property-based tests for flatten join strategy Feature: spark-etl-framework,
  * Property 23: Flatten Join Strategy Validates: Requirements 15.3
  *
...
object FlattenJoinProperties extends Properties("FlattenJoin") {
  ...
  // Helper to apply flatten join using JoinStrategyExecutor directly
  def applyFlattenJoin(
      parent: DataFrame,
      child: DataFrame,
  ...
  /** Property 23: Flatten Join Preserves Parent Columns For any parent-child
    * join with flatten strategy, all parent columns should be preserved.
    */
  property("flatten_join_preserves_parent_columns") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(
            s"Exception in flatten_join_preserves_parent_columns: ${e.getMessage}"
          )
          e.printStackTrace()
          false
      }
  ...
  /** Property 23: Flatten Join Includes Child Columns For any parent-child join
    * with flatten strategy, child columns (excluding join keys) should be
  ...
  property("flatten_join_includes_child_columns") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(
            s"Exception in flatten_join_includes_child_columns: ${e.getMessage}"
          )
          e.printStackTrace()
          false
      }
  ...
  /** Property 23: Flatten Join Handles Column Name Conflicts For any
    * parent-child join with flatten strategy where parent and child have
    * columns with the same name, the child column should be prefixed with
    * "child_".
    */
  property("flatten_join_handles_column_conflicts") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        val childRecords =
          parentIds.take(Math.min(5, parentIds.size)).map { id =>
            (id, "child_value", 42)
          }

        val parentDF = createParentDF(parentRecords)
        val childDF =
          childRecords.toDF("parent_id", "parent_name", "child_specific")
        ...
        resultColumns.contains("parent_name") && resultColumns.contains(
          "child_parent_name"
        )
        ...
        case e: Exception =>
          println(
            s"Exception in flatten_join_handles_column_conflicts: ${e.getMessage}"
          )
          e.printStackTrace()
          false
      }
  ...
  /** Property 23: Flatten Join Inner Join Produces Matching Records Only For
    * any parent-child inner join with flatten strategy, the result should only
    * contain records where parent and child match.
    */
  property("flatten_join_inner_produces_matching_records") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(
            s"Exception in flatten_join_inner_produces_matching_records: ${e.getMessage}"
          )
          e.printStackTrace()
          false
      }
  ...
  /** Property 23: Flatten Join Left Outer Preserves All Parent Records For any
    * parent-child left outer join with flatten strategy, all parent records
    * should be preserved even if they have no matching children.
    */
  property("flatten_join_left_outer_preserves_parents") = forAll(
    ...
    if (parentRecords.isEmpty) {
      ...
      try {
        ...
        case e: Exception =>
          println(
            s"Exception in flatten_join_left_outer_preserves_parents: ${e.getMessage}"
          )
          e.printStackTrace()
          false
      }
