...
/** Property-based tests for validation rule behaviors (reject vs warn) Feature:
  * spark-etl-framework, Property 11: Validation Rule Reject Behavior Feature:
  * spark-etl-framework, Property 12: Validation Rule Warn Behavior Validates:
  * Requirements 8.5, 8.6
  *
  * Test configuration: Uses standardTestCount (20 cases)
  */
object ValidationRuleBehaviorProperties
    extends Properties("ValidationRuleBehavior") {
  ...
  val mixedRecordsGen: Gen[(Seq[TestRecord], Int)] = for {
    ...
    invalidRecords <- Gen.listOfN(
      invalidCount,
      testRecordGen(validEmail = false)
    )
  } yield {
  ...
  /** Property 11: Validation Rule Reject Behavior For any validation rule
    * configured with on_failure: reject, all records that fail the validation
    * should be moved to rejected records
    */
  property("reject_behavior_moves_failed_records_to_rejected") =
    forAll(mixedRecordsGen) { case (records, _) =>
      Try {
        if (records.isEmpty) {
          ...
          val flowConfig =
            flowConfigWithRegexRule(onFailure = OnFailureAction.Reject)

          // Count actual invalid emails in the input
          val emailPattern = "^[a-zA-Z0-9]+@[a-zA-Z0-9]+\\.[a-zA-Z]+$"
          val actualInvalidCount =
            records.count(r => !r.email.matches(emailPattern))
          ...
          val validHasNoInvalidEmails = {
            val validEmails =
              result.valid.select("email").collect().map(_.getString(0))
          ...
          val correctRejectionCode = if (actualInvalidCount > 0) {
            result.rejected.exists { rejectedDf =>
              rejectedDf
                .filter(col(REJECTION_CODE) === "REGEX_VALIDATION_FAILED")
                .count() == actualInvalidCount
            }
          } else {
            true // No rejected records expected
          }
  ...
  /** Property 11: Validation Rule Reject Behavior - Record Count Invariant For
    * any validation with reject behavior, input_count = valid_count +
    * rejected_count
    */
  ...
  /** Property 11: Validation Rule Reject Behavior - No Invalid Records in Valid
    * Set For any validation with reject behavior, valid records should not
    * contain any that fail the rule
    */
  ...
  /** Property 12: Validation Rule Warn Behavior For any validation rule
    * configured with on_failure: warn, all records that fail the validation
    * should pass with a warning added to the warnings column
    */
    ...
    case (records, _) =>
      Try {
        if (records.isEmpty) {
          ...
          val df = createDataFrame(records)
          val flowConfig =
            flowConfigWithRegexRule(onFailure = OnFailureAction.Warn)
  ...
  /** Property 12: Validation Rule Warn Behavior - Warning Content For any
    * validation with warn behavior, warnings should contain descriptive
    * information
    */
  property("warn_behavior_warning_content_descriptive") =
    forAll(mixedRecordsGen) { case (records, _) =>
      Try {
        if (records.isEmpty) {
          true // Skip empty test cases
  ...
  /** Property 12: Validation Rule Warn Behavior - Valid Records Have Empty
    * Warnings For any validation with warn behavior, records that pass should
    * have empty warnings array
    */
  property("warn_behavior_valid_records_empty_warnings") =
    forAll(Gen.choose(5, 30)) { count =>
      Try {
        ...
        val allHaveEmptyOrNoRegexWarnings = result.valid
          ...
          .forall { row =>
            ...
            warnings.isEmpty || !warnings.exists(w =>
              w.toLowerCase.contains("email") && w.toLowerCase
                .contains("pattern")
            )
          }
  ...
  /** Property 12: Validation Rule Warn Behavior - Record Count Invariant For
    * any validation with warn behavior, input_count = valid_count +
    * rejected_count (rejected only for other validation failures, not for the
    * warn rule)
    */
  ...
  /** Property 11 & 12: Multiple Rules with Different Behaviors For any
    * validation with multiple rules having different on_failure settings, each
    * rule should behave according to its configuration
    */
  property("multiple_rules_different_behaviors") = forAll(Gen.choose(5, 30)) {
    count =>
      Try {
        // Create records with some having invalid emails and some having invalid ages
        val records = (1 to count).map { i =>
          val email =
            if (i % 3 == 0) s"invalid_email_$i" else s"user$i@example.com"
          ...
          validation = ValidationConfig(
            ...
            rules = Seq(
              ValidationRule(
                ...
                description = Some("Email must be valid format"),
                skipNull = Some(false),
                onFailure = OnFailureAction.Warn // Warn for email
              ),
              ValidationRule(
                ...
                skipNull = Some(false),
                onFailure = OnFailureAction.Reject // Reject for age
              )
            )
          ),
          output = OutputConfig(
        ...
        val ageRejectionsExist = result.rejected.exists { rejectedDf =>
          rejectedDf
            .filter(col(REJECTION_CODE) === "RANGE_VALIDATION_FAILED")
            .count() > 0
        }
        ...
        val emailWarningsExist = {
          val recordsWithInvalidEmailValidAge = result.valid
            .filter(
              !col("email").rlike("^[a-zA-Z0-9]+@[a-zA-Z0-9]+\\.[a-zA-Z]+$")
            )
  ...
  /** Property 11: Validation Rule Reject Behavior - Empty DataFrame For any
    * empty DataFrame with reject rule, validation should pass without errors
    */
  ...
  /** Property 12: Validation Rule Warn Behavior - Empty DataFrame For any empty
    * DataFrame with warn rule, validation should pass without errors
    */
  property("warn_behavior_empty_dataframe") = forAll(Gen.const(())) { _ =>
    Try {
      val emptyDf = Seq.empty[TestRecord].toDF()
      val flowConfig = flowConfigWithRegexRule(onFailure = OnFailureAction.Warn)
