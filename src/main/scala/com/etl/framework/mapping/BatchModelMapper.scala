...
import com.etl.framework.exceptions.{
  DataFrameToDatasetMappingException,
  MappingConfigLoadException,
...
/** Maps DataFrame to typed Dataset[BatchModel] Supports column mappings and
  * expressions from YAML configuration
  */
class BatchModelMapper[T: Encoder](
  ...
  def map(df: DataFrame): Dataset[T] = {
    logger.info(
      s"Mapping DataFrame to Dataset with ${mappingConfig.mappings.size} field mappings"
    )
    ...
    try {
      ...
      case e: Exception =>
        ...
        throw DataFrameToDatasetMappingException(
          ...
          details =
            s"${e.getMessage}. Ensure the DataFrame schema matches the case class structure.",
          cause = e
        )
    }
  }
    ...
    config.mappings.foldLeft(df) { (currentDf, mapping) =>
      mapping.expression match {
        case Some(exprStr) =>
          // Apply expression
          logger.debug(
            s"Applying expression for field ${mapping.targetField}: $exprStr"
          )
          try {
        ...
        case None =>
          ...
          if (mapping.sourceField != mapping.targetField) {
            logger.debug(
              s"Renaming column ${mapping.sourceField} to ${mapping.targetField}"
            )
            if (currentDf.columns.contains(mapping.sourceField)) {
              currentDf.withColumnRenamed(
                mapping.sourceField,
                mapping.targetField
              )
            } else {
              logger.warn(
                s"Source field ${mapping.sourceField} not found in DataFrame, skipping"
              )
              currentDf
            }
          } else {
...
object BatchModelMapper {
  ...
  def loadMappingConfig(mappingFilePath: String): MappingConfig = {
    ...
    scala.util.Using(Source.fromFile(mappingFilePath)) { source =>
      ...
      YamlConfigSource.string(yamlContent).load[MappingConfig] match {
        case Right(config) =>
        ...
        case Left(failures) =>
          throw MappingConfigLoadException(
            ...
            details =
              s"Failed to decode mapping config: ${failures.toList.map(_.description).mkString("; ")}",
            cause = new RuntimeException(failures.prettyPrint())
          )
      }
    } match {
  ...
  /** Creates a BatchModelMapper with mapping loaded from file
    */
  def fromFile[T: Encoder](
      mappingFilePath: String
  )(implicit spark: SparkSession): BatchModelMapper[T] = {
